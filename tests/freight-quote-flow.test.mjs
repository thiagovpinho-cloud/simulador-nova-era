...
assert.match(orders,/Solicitar cotação à Logística/);
assert.match(orders,/Retorno da Logística/);
assert.match(orders,/MENOR COTAÇÃO/);
assert.match(logistics,/Cotações pendentes/);
assert.match(logistics,/Enviar cotações ao Comercial/);
assert.match(logistics,/Prestador \/ transportadora/);
assert.match(logistics,/ESTOQUE:'inventory'|Cotação solicitada/);
assert.match(logistics,/replace\(\/\\\.\/g,''\)/);
