(function(){
  ...
  function render(state){
    ...
    document.getElementById('fiMov').onclick=()=>openLegacy('movements');
    document.getElementById('fiInv').onclick=()=>openLegacy(s.tab==='inputs'?'inputs':'inventory');
    document.getElementById('fiRepos').onclick=()=>openLegacy('purchases');
  }
  ...
  function openItem(tab,key){
    ...
    setTimeout(()=>{document.querySelector('#opsBody [data-view="'+(tab==='inputs'?'inputs':'inventory')+'"]')?.click();setTimeout(()=>document.querySelector('['+(tab==='inputs'?'data-move-input':'data-move-finished')+'="'+CSS.escape(key)+'"]')?.click(),20)},20);
  }
  function openLegacy(view){document.getElementById('focadoShell')?.classList.add('hidden');const btn=document.getElementById('hubGoOperacoes');if(!btn)return;btn.click();setTimeout(()=>document.querySelector('#opsBody [data-view="'+view+'"]')?.click(),20)}
  window.FocadoInventory={render,openItem,openLegacy};
})();