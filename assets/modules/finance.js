(function(){
  ...
  function netValue(o,f){return orderValue(o)-['taxes','discounts','returns','bonuses'].reduce((s,k)=>s+Number(f?.[k]||0),0)}
  function status(o,f){
  ...
  function render(){
    ...
    root().innerHTML='<div class="ffin-page">'+
      ...
      rows.map(o=>{const f=facts.get(String(o.id)),st=status(o,f);return '<tr><td><b>'+esc(o.number||o.id)+'</b></td><td>'+esc(o.client||'')+'</td><td>'+money(orderValue(o))+'</td><td>'+esc(f?.invoice_number||'—')+'</td><td>'+esc(f?.invoice_date||'—')+'</td><td>'+money(netValue(o,f))+'</td><td><span class="ffin-chip '+st[1]+'">'+st[0]+'</span></td><td><button class="ffin-link" data-edit="'+esc(o.id)+'">Abrir</button></td></tr>'}).join('')+
      '</tbody></table></div></div>';
  ...
  function openOrder(id){
    ...
    root().innerHTML='<div class="ffin-page"><div class="ffin-head"><div><button class="ffin-btn" id="ffinBack">← Financeiro</button><h1>'+esc(o.number||o.id)+'</h1><p>'+esc(o.client||'')+' · Bruto '+money(orderValue(o))+'</p></div></div>'+
      '<div class="ffin-grid">'+
        ...
        '</div></section>'+
        '<section class="ffin-card"><h2>Composição financeira</h2><div class="ffin-form two">'+
          numberField('Impostos','ffinTaxes',f.taxes)+
        ...
        '</div><div class="ffin-summary"><span>Bruto</span><b>'+money(orderValue(o))+'</b><span>Líquido atual</span><b>'+money(netValue(o,f))+'</b></div></section>'+
      '</div>'+
    ...
    $('#ffinSave').onclick=async()=>{
      const fact={
        ...
        taxes:$('#ffinTaxes').value,discounts:$('#ffinDiscounts').value,returns:$('#ffinReturns').value,
        bonuses:$('#ffinBonuses').value,commission:$('#ffinCommission').value,freight_allocated:$('#ffinFreight').value
      };
